...
/*
 * Sort alphabetically by lastName (but also see the next snippet!)
 * Array.prototype.toStored((a, b) => number)
 * returns a sorted array (Array.prototype.sort would sort the array in-place)
 ...
 * strings in ascending alphabetical order are less than those that are lower
 */
let sortedPeople = people.toSorted((p1, p2) => {
	if (p1.lastName > p2.lastName) {
		return 1; // 'ab' has a greater value than 'aa' and 'ab' should sort last
	} else if (p1.lastName < p2.lastName) {
		return -1; // 'aa' has a lesser value than 'ab' and should sort first
	} else {
		return 0;
	}
});
console.log('Custom sort function: ', sortedPeople);

/*
 * Sort using Intl.Collator
 * Intl.Collator(languageCode).compare builds a compare function appropriate
 * to the language code passed in.
 *
 */
let compare = new Intl.Collator('en').compare;
let collatedPeople = people.toSorted((p1, p2) => {
	return compare(p1.lastName, p2.lastName);
});
console.log('Sort using Intl.Collator: ', collatedPeople);
